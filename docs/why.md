...
`go-ruby-openstack/openstack` reimplements Ruby's `openstack` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-openstack/openstack` directly, with no Ruby runtime;
...
Because `openstack` is pure Go with cgo disabled, it also compiles to **WebAssembly**
